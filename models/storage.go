...
func (swu *StorageFilesWithUser) ListCurrentFile(id string) (*StorageFile, error) {
	file := StorageFile{}
...
func (swu *StorageFilesWithUser) DeleteFileFromID(id string) error {
	file := &StorageFile{}
	err := GetDB().Model(&StorageFile{}).Where("id=? and user_id=?", id, swu.Owner.ID).First(file).Error
	if err != nil {
		return err
	}
	if err = GetDB().Where("id=? ", id).Or("parent_id = ?", id).Delete(StorageFile{}).Error; err != nil {
		return err
	}
	return nil
}
